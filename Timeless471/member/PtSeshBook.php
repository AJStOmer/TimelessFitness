...
<html>
    ...
    <body class="parent">
        ...
        <form action="bookSuccess.php" method="post" class="cForm">
            <h1 class="Scourt">Select a PT Session</h1>
            <select name= "bookableSelect" class="CourtSelections" multiple>
                    <?php
                        ...
                        $seshresult = mysqli_query($con, "SELECT distinct PT_Session.Session_Focus, Person.Fname, Person.Lname, Person.email FROM PT_Session, Person Where PT_Session.PT_email = Person.email");
                        $count = 0;
                        while($srow = mysqli_fetch_array($seshresult)){
                            $sname = $srow['Session_Focus'];
                            $ptFName = $srow['Fname'];
                            $ptLName = $srow['Lname'];
                            $ptemail = $srow['email'];
                            if($count == 0){
                                echo "<option value='$sname' selected>$sname by $ptFName $ptLName</option>";
                            }else{
                                echo "<option value='$sname'>$sname by $ptFName $ptLName</option>";
                            }
                            $count++;
                        }
                    ?>
            ...
            ?>
                ...
                <input type="submit" name='choice' class="machineBooksubmit" value="Book Session">
                <?php
